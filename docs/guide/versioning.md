...
> Баннеры теперь настраиваются гибко — разные сообщения для актуальной и каждой старой версии, а также глобальные баннеры-объявления. См. [Баннеры](/guide/banners). Поле `oldVersionBanner` ниже помечено как deprecated и сохранено для совместимости.
...
| `latestBanner` | `DocusiteBanner \| false?` | — | Баннер на страницах актуальной версии (см. [Баннеры](/guide/banners)) |
| `oldVersionBanner` | `object?` | — | ⚠️ _deprecated_. Fallback-баннер для старых версий без своего `banner`. Используйте per-version `banner` и `latestBanner` |
...
| `banner` | `DocusiteBanner \| false?` | Баннер для этой версии. `false` — явно отключить (см. [Баннеры](/guide/banners)) |

## Баннер старой версии

> Гибкая настройка баннеров описана в разделе [Баннеры](/guide/banners). Ниже — устаревший вариант через `oldVersionBanner`.
...
> ⚠️ `oldVersionBanner` помечен как deprecated. Чтобы полностью отключить баннеры для старых версий, используйте `banner: false` на каждом элементе `older[]` (см. [Баннеры](/guide/banners)).
...
> ⚠️ `oldVersionBanner` помечен как deprecated. Рекомендуется per-version `banner` с плейсхолдерами `{latestLink}`, `{latestLabel}`, `{versionLabel}` (см. [Баннеры](/guide/banners)).

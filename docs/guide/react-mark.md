...
Vue-компонент попадает в клиентский бандл темы только если `<ReactMark />` реально используется в markdown (примеры в code fence не считаются). В `nav`/`sidebar` плейсхолдер всегда заменяется на инлайн SVG на этапе генерации конфига — отдельный Vue-компонент для этого не нужен.

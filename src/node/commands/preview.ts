...
export async function preview(root?: string, port?: number) {
  ...
  const { config: vpConfig, versions, versionsLatestLink, changelogSrc, contentInjections, runtimeScriptCode, hasI18n, frameworkMarks } = transformConfig(resolvedConfig, docsDir)
  writeVitePressConfig(docsDir, vpConfig, { versions, versionsLatestLink, changelogSrc, contentInjections, runtimeScriptCode, hasI18n, frameworkMarks })

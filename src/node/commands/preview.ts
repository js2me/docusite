...
export async function preview(root?: string, port?: number) {
  ...
  const { config: vpConfig, versions, versionsLatestLink, changelogSources, changelogTabs, contentInjections, runtimeScriptCode, hasI18n, frameworkMarks } = transformConfig(resolvedConfig, docsDir)
  writeVitePressConfig(docsDir, vpConfig, { versions, versionsLatestLink, changelogSources, changelogTabs, contentInjections, runtimeScriptCode, hasI18n, frameworkMarks })
